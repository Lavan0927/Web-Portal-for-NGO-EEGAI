...
function TopUnRegisteredDonorsTable() {
  const [topDonors, setTopDonors] = useState([]);

  useEffect(() => {
    // Fetch the top donors' information from the backend API
    axios.get("/api/getTopUnRegisteredDonors").then((response) => {
  ...
  const columns = [
    { field: "nicNo", headerName: "Nic No", flex: 1 },
    { field: "fullname", headerName: "Donor Name", flex: 1 },
    {
...
export default TopUnRegisteredDonorsTable;
